...
- There is debates about the relative benefits of reporting p-values versus confidence intervals (CI's) (though both are often reported).
- In public health research, CI's are favored. 
...
- Step 2: Specify the null hypothesis value, $H_0$, pronounced "H naught", of that parameter
...
* The $\alpha$ level is the desired type I error rate.  This is the tolerable probability of false positives (rejection the null when it is true) in the theoretical context of repeated experiments (optional).
...
**Step 6**: Thus, under the null hypothesis, this is a 0.28% chance of observing a sample
...
2*pnorm(-1.202,mean=0,sd=1,lower.tail=T)
pnorm(-1.202,mean=0,sd=1,lower.tail=T)+pnorm(q = 1.201666, lower.tail = F)
```
...
* In fact, a 95/% CI can be also derived as the set of null values such that two-sided tests fail to reject at $\alpha=0.05$.
