...
```{r, echo=F, out.width="80%", message=F}
p1 <- ggplot(data = data.frame(x = c(-3, 3)), aes(x)) +
  stat_function(fun = dnorm, n = 101, args = list(mean = 0, sd = 1), col = "orange") +
  labs(y = "density", x = "") + 
  geom_text(aes(x = 0, y = 0.45), label = "mean = 0 \nsd = 1", check_overlap = T, col = "orange") +
...
p2 <- ggplot(data = data.frame(x = c(-3, 3)), aes(x)) +
  ...
  labs(y = "P(X < x)", x = "") + 
  theme_minimal(base_size = 15) +
  geom_segment(aes(x = 0 , y = 0, xend = 0, yend = pnorm(0,0,1))) +
  geom_segment(aes(x = 1 , y = 0, xend = 1, yend = pnorm(1,0,1)), lty = 2) +
  geom_segment(aes(x = -1 , y = 0, xend = -1, yend = pnorm(-1,0,1)), lty = 2) +
  geom_segment(aes(x = 2 , y = 0, xend = 2, yend = pnorm(2,0,1)), lty = 3) +
  geom_segment(aes(x = -2 , y = 0, xend = -2, yend = pnorm(-2,0,1)), lty = 3) +
  scale_x_continuous(breaks = -3:3, 
                     labels = c(-3, "-2\n-2 SD", "-1\n-1 SD", "0\nmean", "1\n+1 SD", "2\n+2 SD", 3))
library(cowplot)
