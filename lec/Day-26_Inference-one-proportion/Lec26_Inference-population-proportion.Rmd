...
- This confidence interval can perform poorly, meaning that if we repeated the confidence interval 100 times (based on 100 random samples), the coverage could be what is termed overly conservative (e.g., coverage significantly above 95\%) or anti-conservative (less than 95 of the 95% confidence intervals would contain the true value for the proportion $p$). We typically say that such a method has poor **coverage** because it contains the true propability $p$ less than the specified amount (typically less than 95\%).
...
sample size by 4 imaginary trials), the interval can have better performance.
...
- Use this method when $n$ is at least 10.
...
- Note, if the number of successes and failures is relatively large, then the $+4$ method will result in the equivalent coverage (CI) as not using it.
...
- It can be thoughth of as an inversion of hypothesis testing.
  - That is, it finds the sets of all values of the parameter that ($p_{null}$), if consider the null value in a two-sided hypothesis test, would lead to a $p-value \ge 0.05$.
  - It calculates the p-value using the binomial distribution function.
  - As a reminder, this was the cumulative probabiity function for binomial (Lec 16):

$$ P(X <= x)=\sum_{i=0}^x {n \choose i}p_{null}^i(1-p_{null})^{n-i}$$
...
This is because it has larger coverage (contains the true value more often) which
